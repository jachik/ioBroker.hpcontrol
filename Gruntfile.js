...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'Jacek Schikora';
    var email     = grunt.option('email')  || 'js@j-s-edv.de';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new hpcontrol name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/hpcontrol.png')) {
            fs.renameSync(__dirname + '/admin/hpcontrol.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/hpcontrol.html')) {
            fs.renameSync(__dirname + '/widgets/hpcontrol.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/hpcontrol/js/hpcontrol.js')) {
            fs.renameSync(__dirname + '/widgets/hpcontrol/js/hpcontrol.js', __dirname + '/widgets/hpcontrol/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/hpcontrol')) {
            fs.renameSync(__dirname + '/widgets/hpcontrol',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Hpcontrol/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Hpcontrol'
                        },
                        {
                            match: /Jacek Schikora/g,
                            replacement: author
                        },
                        {
                            match: /js@j-s-edv.de/g,
                            replacement: email
                        }
                    ]
                },
